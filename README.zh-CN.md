...
它的目标不是做一个无限制的“安全 Agent”，而是做一个可维护、可集成、agent 真能用起来的代码安全扫描层。
...
它暂时不适合被宣传成成熟企业级 SAST 平台的替代品。现阶段最强的定位，是高杠杆的安全审查助手，以及 agent-native 的扫描能力层。

## 5 分钟体验

如果你第一次打开这个仓库，建议先看这三样东西：
...
这里故意保持最短路径。一个工具能不能被用起来，第一条真实路径是否顺滑很重要。
...
如果你想让 CodeScan 在 Codex 里更像“原生能力”，推荐两层一起用：
...
如果你希望一个开源工具真的被别人用起来，除了代码本身，文档、示例、测试、贡献体验都很重要。
